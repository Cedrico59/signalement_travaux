...
function renderListBasic_(list) {
  // essaie d'utiliser ta fonction existante
  if (typeof renderList === "function") { try { renderList(); return; } catch(e){} }
  // sinon: pas de rendu ici (ta liste existante reste)
}

let toggleLayer_ = null;
function renderMarkersBasic_(list) {
  const theMap = window._leafletMap || window.map || (typeof map !== "undefined" ? map : null);
  if (!theMap || typeof L === "undefined") return;
  if (!toggleLayer_) toggleLayer_ = L.layerGroup().addTo(theMap);
  toggleLayer_.clearLayers();
  ...
  for (const r of list) {
    ...
    const m = L.marker([lat, lng]).addTo(toggleLayer_);
...
async function reloadReportsOrArchives_() {
  ...
  if (data && Array.isArray(data.reports)) {
    // remet dans la variable globale si elle existe
    if (typeof reports !== "undefined") {
      reports = data.reports;
    } else {
      window.reports = data.reports;
    }
    renderMarkersBasic_(data.reports);
    renderListBasic_(data.reports);
  }
}
