...
(() => {
  ...
  // =========================
  // Si vous avez dj une WebApp Google Apps Script comme lappli Patrimoine arbor,
  // vous pouvez mettre lURL ici pour activer login + sync.
  // Lapp fonctionne aussi 100% en local si API_URL = "".
  const STORAGE_KEY = "signalement_travaux_v1";
  
  //  Google Apps Script WebApp (API)
  // Collez ici l'URL de la WebApp dploye (Apps Script).
  ...
  const SECTEURS = ["Hautes Loges - Briqueterie", "Bourg", "Buisson - Delcencerie", "Mairie - Quesne", "Pont - Plouich - Clmenceau", "Cimetire Delcencerie", "Cimetire Pont", "Ferme aux Oies"];
...
async function apiPost(action, data) {
  ...
  //  Apps Script: viter le preflight CORS => PAS de Content-Type JSON
...
async function openHistoryAdmin() {
  if (!isAdmin()) { alert("Historique rserv admin"); return; }
...
const DEFAULT_CENTER = [50.676, 3.086];
  ...
  // Numrotation centralise via Apps Script (anti-doublons)
  // Si pas connect -> fallback compteur local (dmo/hors-ligne)
  const sess = loadSession();
...
//  Contour EXACT de Marcq-en-Barul (source : geo.api.gouv.fr / geometry=contour)
  const MARCQ_POLYGON = [
    [50.653496, 3.118930],
    [50.653182, 3.115914],
    [50.652712, 3.116315],
  ...
  //  Emprise (bbox) utilise pour bloquer le dplacement de carte
...
function showUndoBar(message) {
  ...
  if (!bar) {
    ...
    // IMPORTANT : crer aussi les lments avec createElement (pas innerHTML fragile)
...
async function undoDelete() {
  ...
  console.warn("Suppression serveur choue", e);
...
const doneBtn = () => el("doneBtn");
  ...
  //  Couleurs secteurs (IDENTIQUES  Patrimoine arbor)
  const SECTOR_COLORS = {
    "Hautes Loges - Briqueterie": "#1565C0",
    "Bourg": "#2E7D32",
    "Buisson - Delcencerie": "#EF6C00",
    "Mairie - Quesne": "#6A1B9A",
    "Pont - Plouich - Clmenceau": "#00838F",
    "Cimetire Delcencerie": "#4E342E",
    "Cimetire Pont": "#C62828",
    "Ferme aux Oies": "#546E7A"
  };

  // =========================
  // LGENDE SECTEURS (rtractable)
  // HTML attendu :
  ...
  function buildSectorLegend() {
    ...
    box.innerHTML = `
      <div class="legend-header">
        <span> Secteurs</span>
        <button type="button" class="legend-toggle" id="legendToggleBtn">${collapsed ? "Ouvrir" : "Rduire"}</button>
      </div>
  ...
  try {
    ...
    // On ignore les lments hors Marcq
...
function getById(id) { return reports.find(r => r.id === id); }
  ...
  const sectorColors = {
    ...
    "Pont - Plouich - Clmenceau": "#06b6d4",
    "Cimetire Delcencerie": "#ef4444",
    "Cimetire Pont": "#eab308",
...
const dotColor = (String(r.interventionType || "").toLowerCase().startsWith("ex"))
  ...
  //  Croix blanche si travaux effectus (bouton accessible  tous)
  ...
  const html = `
    <div class="marker-wrap">
      <svg width="36" height="36" viewBox="0 0 36 36" aria-hidden="true">
        ...
        <!-- croix blanche (travaux effectus) -->
        ${showCross ? `
...
function removeMarker(id) {
  ...
  function updatePhotoStatus() {
    ...
    photoStatusEl().textContent = ` ${pendingPhotos.length} photo${pendingPhotos.length>1?"s":""} ajoute${pendingPhotos.length>1?"s":""}`;
  }
...
async function toggleDone(id) {
  ...
  saveLocal();
  renderAll();
  showToast(r.done ? "Travaux effectus " : "Travaux rouverts ");
  ...
  try {
    ...
    console.warn("Sync done choue", e);
  }
}
...
<p><b>Date dexcution :</b> ${escapeHtml(r.dateExecution || "")}</p>
<p><b>Nature :</b> ${escapeHtml(r.nature || "")}</p>
<p><b>Coordonnes :</b> ${fmtCoord(r.lat)}, ${fmtCoord(r.lng)}</p>
<p><b>Commentaire :</b></p>
<small>${escapeHtml(r.comment || "")}</small>
  ...
  function setSelected(id) {
    ...
    if (r && !canSeeReport(r)) { alert("Accs interdit  ce secteur."); return; }

    if (!r) {
      editorTitle().textContent = "Ajouter un signalement";
      editorHint().textContent = "Clique sur la carte pour choisir lemplacement, puis complte la fiche.";
  ...
  if (doneBtn) doneBtn().disabled = false;
    ...
    // Date demande dintervention (admin)
  ...
  function renderList() {
    ...
    if (filtered.length === 0) {
      ...
      empty.textContent = q ? "Aucun rsultat." : "Aucun signalement. Clique sur la carte pour en ajouter un.";
    ...
    for (const r of filtered) {
      ...
      title.textContent = r.nature || "Travaux (nature non prcise)";
  ...
  function initMap() {
    ...
    // Affichage du contour Marcq-en-Barul (comme Patrimoine arbor)
...
function handleMapSelect(e) {
      if (!isInMarcq(e.latlng.lat, e.latlng.lng)) {
        alert(" Le signalement doit tre situ  Marcq-en-Barul.");
        return;
      }
  ...
  if (doneBtn) doneBtn().disabled = true;
      editorTitle().textContent = "Ajouter un signalement";
      editorHint().textContent = "Complte la fiche puis clique sur Enregistrer.";
  ...
  function locateUserGPS() {
    if (!navigator.geolocation) { alert("La golocalisation nest pas supporte sur cet appareil."); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (!isInMarcq(lat, lng)) {
          alert(" Votre position GPS est hors Marcq-en-Barul.");
          return;
        }
  ...
  if (doneBtn) doneBtn().disabled = true;
        ...
        editorHint().textContent = "Position GPS dtecte automatiquement.";
      ...
      () => alert("Impossible dobtenir la position GPS."),
  ...
  async function buildFromForm(existing=null) {
    const lat = parseFloat(latEl().value);
    const lng = parseFloat(lngEl().value);
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new Error("Coordonnes invalides (clique sur la carte ou GPS).");

    if (!isInMarcq(lat, lng)) {
      throw new Error("Le signalement doit tre situ  Marcq-en-Barul.");
    }
...
function wireUI() {
  applySessionUI(loadSession());
    ...
    console.log('SAVE CLICK');
      ...
      try {
        ...
        try {
          const sess = loadSession();
          if (apiEnabled() && sess) {
            //  Enregistre/Met  jour le signalement ct Google Sheets
    ...
    await apiPost("saveReport", {
            ...
            //  Re-synchronisation depuis le serveur (source de vrit)
            await refreshFromServer();
            showToast(" Enregistr");
          }
        } catch (e) {
          console.warn("Sync serveur choue", e);
  ...
  if (!isAdmin()) {
    alert("Suppression rserve aux administrateurs.");
    return;
  }
  ...
  try {
    ...
    toggleListBtn().onclick = () => {
      ...
      toggleListBtn().textContent = open ? "Ouvrir" : "Rduire";
    };
...
function renderMarkers() {
  // Nettoie tous les marqueurs affichs
  for (const m of markers.values()) map.removeLayer(m);
  markers.clear();

  // Recre uniquement ceux visibles
  ...
  function buildFicheText(r) {
    return `Signalement de travaux  Marcq-en-Barul

Numro de dossier : ${r.dossierNumber || ""}
ID : ${r.id}
Secteur : ${r.secteur}
...
`;
  ...
  async function exportPDF() {
    ...
    pdf.setFontSize(14);
    pdf.text("Signalement de travaux  Ville de Marcq-en-Barul", pageWidth / 2, 18, { align: "center" });
    ...
    line("Numro de dossier", r.dossierNumber || "");
    ...
    line("Date dexcution", r.dateExecution || "");
    line("Nature des travaux", r.nature || "");
    line("Coordonnes", `${r.lat?.toFixed(6)}, ${r.lng?.toFixed(6)}`);
  ...
  function printFiche() {
    ...
    const html = `
      ...
      <body>
        <h1>Signalement de travaux  Marcq-en-Barul</h1>
        <div class="box">
          <p><b>Numro de dossier :</b> ${r.dossierNumber || ''}</p>
          <p><b>ID :</b> ${r.id}</p>
          ...
          <p><b>Date excution :</b> ${r.dateExecution}</p>
          <p><b>Nature :</b> ${r.nature}</p>
          <p><b>Commentaire :</b><br>${r.comment}</p>
          <p><b>Coordonnes :</b> ${r.lat.toFixed(6)}, ${r.lng.toFixed(6)}</p>
        </div>
      </body></html>
    `;
...
// ===============================
// Toggle Archives/Reports - reload direct GAS (independant du reste)
// ===============================
...
function getToken_() {
  try {
    ...
    const sessRaw = localStorage.getItem("session");
    if (sessRaw) {
      try {
        const s = JSON.parse(sessRaw);
        if (s && s.token) candidates.push(s.token);
        if (s && s.auth && s.auth.token) candidates.push(s.auth.token);
      } catch(e){}
    }

    candidates.push(localStorage.getItem("token"));
    candidates.push(localStorage.getItem("authToken"));
    candidates.push(localStorage.getItem("jwt"));
    ...
    candidates.push(window.authToken);
    candidates.push(window.SESSION && window.SESSION.token);
...
function findToggleBtn_() {
  const ids = ["toggleArchivesBtn","toggleArchivesBtn","toggleArchives","btnArchives","toggleArchivesButton"];
  for (const id of ids) {
  ...
  return btns.find(b => /archives/i.test(b.textContent || "")) || null;
}
...
function ensureToggleBtn_() {
  ...
  // Try to attach into top bar
  const header = document.querySelector(".topbar") || document.querySelector("header") || document.body;
